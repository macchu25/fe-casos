...
export default function ProfilePage() {
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    if (!profileFormData.name) {
      showToast(t('profile.fullNameLabel') + " error", "error");
      return;
    }
  ...
  const handleSaveContact = async (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.phone) {
      showToast("Please check fields.", "error");
      return;
    }
  ...
  const handleDeleteContact = async (id: string) => {
    const confirmed = await confirm(t('profile.confirmDeleteTitle'), t('profile.confirmDeleteDesc'));
    if (!confirmed) return;
  ...
  const handleTestCall = async (phone: string) => {
    const token = (session?.user as any)?.accessToken;
    showToast(`Test call to ${phone}...`, "info");
    ...
    try {
      ...
      if (res.ok) {
        showToast("Success! Check phone.", "success");
      } else {
  ...
  return (
    <div className="dashboard-section" style={{ minHeight: '100vh' }}>
      <header className="page-header-premium">
        <div>
          <h1 className="page-title-premium">{t('profile.title')}</h1>
      ...
      <div className="profile-grid">
        ...
        <section className="overview-card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', position: 'relative' }}>
          ...
          <div style={{ color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '8px', justifyContent: 'center', marginBottom: '40px', fontWeight: 600, fontSize: '1.1rem' }}>
            <Calendar size={20} /> {patient.age || 'N/A'} {t('profile.yearsOld')} ({t('profile.subject')})
          </div>
          ...
          <div style={{ width: '100%', textAlign: 'left', display: 'flex', flexDirection: 'column', gap: '24px', background: 'var(--bg-primary)', padding: '32px', borderRadius: '24px', border: '1px solid var(--border)' }}>
            <div style={{ display: 'flex', gap: '20px', alignItems: 'center' }}>
              ...
              <div>
                <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', fontWeight: 500 }}>{t('profile.locationLabel')}</div>
                <span style={{ fontWeight: 700, fontSize: '1.1rem', color: 'var(--text-main)' }}>{patient.location || t('profile.unknownLocation')}</span>
              </div>
            </div>
            <div style={{ display: 'flex', gap: '20px', alignItems: 'center' }}>
              ...
              <div>
                <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', fontWeight: 500 }}>{t('profile.alarmChannelLabel')}</div>
                <span style={{ fontSize: '1.1rem', color: 'var(--text-main)', fontWeight: 700 }}>{patient.bloodType || t('profile.unknownChannel')}</span>
              </div>
            </div>
          </div>
          ...
          >
            <Sliders size={18} /> {t('profile.configAiBtn')}
          </button>
        </section>
          ...
          <section className="overview-card">
            ...
            <div className="responsive-grid-2col-equal" style={{ marginBottom: '24px' }}>
              <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#64748b', fontSize: '0.85rem', fontWeight: 600, marginBottom: '6px' }}>
                  ...
                  {t('profile.thrLowLabel')}
                </div>
                <div style={{ fontSize: '1.25rem', fontWeight: 800, color: '#1e293b' }}>
                  {patient.thrLow || 0.015} <span style={{ fontSize: '0.85rem', color: '#64748b', fontWeight: 500 }}>{t('profile.gforce')}</span>
                </div>
              </div>
              <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#64748b', fontSize: '0.85rem', fontWeight: 600, marginBottom: '6px' }}>
                  <Shield size={14} color="#ef4444" />
                  {t('profile.thrHighLabel')}
                </div>
                <div style={{ fontSize: '1.25rem', fontWeight: 800, color: '#1e293b' }}>
                  {patient.thrHigh || 0.040} <span style={{ fontSize: '0.85rem', color: '#64748b', fontWeight: 500 }}>{t('profile.gforce')}</span>
                </div>
              </div>
            </div>
            ...
            <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '16px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '28px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <div>
                  <div style={{ fontWeight: 700, fontSize: '0.95rem', color: '#1e293b' }}>{t('profile.sirenTitle')}</div>
                  <div style={{ fontSize: '0.8rem', color: '#64748b', fontWeight: 500 }}>{t('profile.sirenDesc')}</div>
                </div>
              </div>
              <span style={{
                ...
                {patient.audioAlert ? t('profile.statusOn') : t('profile.statusOff')}
              </span>
            </div>
            ...
            <div style={{ paddingTop: '24px', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ fontSize: '0.9rem', color: 'var(--text-muted)', fontWeight: 500 }}>{t('profile.lastIncidentLabel')}</div>
                <div style={{ fontWeight: 800, fontSize: '1.25rem', color: 'var(--danger)', marginTop: '4px' }}>{patient.lastIncident || t('profile.noIncidents')}</div>
              </div>
                ...
                <div key={contact.id || `contact-${idx}`} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--bg-primary)', padding: '20px 24px', borderRadius: '20px', border: '1px solid var(--border)' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                    ...
                    <button onClick={() => handleTestCall(contact.phone)} style={{ background: 'var(--accent)', color: 'white', border: 'none', padding: '6px 12px', borderRadius: '8px', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <Phone size={14} /> {t('profile.testAiCall')}
                    </button>
                    <div style={{ height: '32px', width: '1px', background: 'var(--border)', margin: '0 8px' }}></div>
                    <button onClick={() => openModal(contact)} style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '10px', cursor: 'pointer', padding: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center' }} title={t('profile.edit')}>
                      <Edit2 size={18} color="var(--accent)" />
                    </button>
                    <button onClick={() => handleDeleteContact(contact.id)} style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '10px', cursor: 'pointer', padding: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center' }} title={t('profile.delete')}>
          ...
          <div style={{
            ...
            <div style={{ marginBottom: '24px' }}>
              <h3 style={{ margin: 0, fontSize: '1.35rem', fontWeight: 800, color: '#1e293b', letterSpacing: '-0.02em' }}>
                {editingContact ? t('profile.editContactTitle') : t('profile.addContactTitle')}
              </h3>
              <p style={{ color: '#64748b', fontSize: '0.85rem', margin: '4px 0 0 0' }}>
                {t('profile.contactModalDesc')}
              </p>
            </div>
            ...
            <form onSubmit={handleSaveContact} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              <div>
                <label style={{ display: 'block', fontSize: '0.88rem', color: '#475569', marginBottom: '6px', fontWeight: 600 }}>{t('profile.fullNameLabel')}</label>
                <input
                  type="text"
                  value={formData.name}
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.88rem', color: '#475569', marginBottom: '6px', fontWeight: 600 }}>{t('profile.phoneLabel')}</label>
                <input
                  type="text"
              ...
              <div style={{ position: 'relative' }}>
                <label style={{ display: 'block', fontSize: '0.88rem', color: '#475569', marginBottom: '6px', fontWeight: 600 }}>{t('profile.relationLabel')}</label>
          ...
          <div style={{
            ...
            <form onSubmit={handleSaveProfile} style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
              ...
              <div className="responsive-grid-profile-main">
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.88rem', color: '#475569', marginBottom: '6px', fontWeight: 600 }}>{t('profile.yearsOld')}</label>
                  <input
                    type="number"
              ...
              <div className="responsive-grid-2col-equal" style={{ gap: '14px' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '0.88rem', color: '#475569', marginBottom: '6px', fontWeight: 600 }}>{t('profile.locationLabel')}</label>
                  <input
                    type="text"
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.88rem', color: '#475569', marginBottom: '6px', fontWeight: 600 }}>{t('profile.alarmChannelLabel')}</label>
                  <input
                    type="text"
                    value={profileFormData.bloodType}
                    onChange={e => setProfileFormData({ ...profileFormData, bloodType: e.target.value })}
                    placeholder="E.g., Telegram + AI Call"
                    style={{
                      width: '100%',
                      padding: '11px 15px',
                      borderRadius: '12px',
              ...
              {/* Cameras conditions */}
              <div>
                <label style={{ display: 'block', fontSize: '0.88rem', color: '#475569', marginBottom: '6px', fontWeight: 600 }}>{t('profile.linkedCamsLabel')}</label>
                <input
                  type="text"
              ...
              <div className="responsive-grid-2col-equal" style={{ gap: '14px' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '0.88rem', color: '#475569', marginBottom: '6px', fontWeight: 600 }}>{t('profile.thrLowLabel')}</label>
                  <input
                    type="number"
                    step="0.001"
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.88rem', color: '#475569', marginBottom: '6px', fontWeight: 600 }}>{t('profile.thrHighLabel')}</label>
                  <input
                    type="number"
                    step="0.001"
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#f8fafc', padding: '16px', borderRadius: '16px', border: '1px solid #e2e8f0', marginTop: '4px' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '0.92rem', color: '#1e293b', fontWeight: 700 }}>{t('profile.sirenTitle')}</label>
                  <span style={{ display: 'block', fontSize: '0.78rem', color: '#64748b', fontWeight: 500, marginTop: '2px' }}>{t('profile.sirenDesc')}</span>
                </div>
                <div
